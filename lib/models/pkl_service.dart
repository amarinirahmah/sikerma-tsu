...
class PklService {
  final String baseUrl = 'http://192.168.100.238:8000/api'; // Ganti dengan IP backend-mu
  ...
  Future<void> uploadPkl(Pkl pkl, {File? file}) async {
    ...
    if (file == null) {
      ...
      final request = http.MultipartRequest('POST', uri)
        ...
        ..fields.addAll(pkl.toJson().map((key, value) => MapEntry(key, value.toString())))
        ..files.add(await http.MultipartFile.fromPath('file_pkl', file.path, filename: basename(file.path)));
  ...
  Future<void> updatePkl(String id, Pkl pkl, {File? file}) async {
    ...
    if (file != null) {
      ...
      request.fields.addAll(pkl.toJson().map((key, value) => MapEntry(key, value.toString())));
      request.files.add(
        await http.MultipartFile.fromPath('file_pkl', file.path, filename: basename(file.path)),
      );
