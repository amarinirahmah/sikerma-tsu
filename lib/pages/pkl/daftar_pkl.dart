...
class RegisterPKLPage extends StatefulWidget {
  const RegisterPKLPage({super.key});

  @override
  State<RegisterPKLPage> createState() => _RegisterPKLPageState();
}

class _RegisterPKLPageState extends State<RegisterPKLPage> {
  final List<_StepData> steps = [
    _StepData(
      'Login / Register Akun',
      'Siswa login atau registrasi ke sistem.',
      Icons.person,
      Colors.green,
    ),
    _StepData(
      'Registrasi oleh Admin',
      'Admin membantu registrasi akun siswa.',
      Icons.admin_panel_settings,
      Colors.orange,
    ),
    _StepData(
      'Pengajuan PKL',
      'Siswa mengajukan permohonan PKL.',
      Icons.upload_file,
      Colors.teal,
    ),
    _StepData(
      'Proses Dokumen',
      'Dokumen kerjasama PKL diproses.',
      Icons.description,
      Colors.deepPurple,
    ),
    _StepData(
      'Pemantauan Status',
      'Siswa memantau status pengajuan.',
      Icons.school,
      Colors.indigo,
    ),
    _StepData(
      'Verifikasi Pengajuan',
      'Admin memverifikasi pengajuan PKL.',
      Icons.verified_user,
      Colors.blue,
    ),
    _StepData(
      'Penerimaan Siswa PKL',
      'Informasi penerimaan ditampilkan.',
                              ...
                              steps
                                  .map(
                                    (step) => Padding(
                                      ...
                                      child: _buildStepItem(step, true),
                                    ),
                                  )
                                  .toList(),
                        )
                              ...
                              child: Row(
                                children: List.generate(steps.length * 2 - 1, (
                                  ...
                                  if (index.isEven) {
                                    ...
                                    return _buildStepItem(step, false);
                                  } else {
                                    return Container(
                                      width: 40,
                                      height: 4,
