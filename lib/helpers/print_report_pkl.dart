...
class CetakLaporanPKLButton extends StatelessWidget {
  final List<dynamic> displayedRows;
  final String judulLaporan;
  final DateTime? startDate;
  final DateTime? endDate;

  const CetakLaporanPKLButton({
    Key? key,
    required this.displayedRows,
    this.judulLaporan = 'Laporan Data Siswa PKL',
    this.startDate,
    this.endDate,
  }) : super(key: key);
  ...
  Future<Uint8List> _generatePdfBytes(List<dynamic> rows, String title) async {
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (context) {
          final prosesCount =
              rows.where((m) => m['statusText'] == 'Diproses').length;
          final setujuCount =
              rows.where((m) => m['statusText'] == 'Disetujui').length;
          final tolakCount =
              rows.where((m) => m['statusText'] == 'Ditolak').length;
          ...
          final tableData = <List<String>>[
            ...
            ...rows.asMap().entries.map((entry) {
              ...
              final nisn = _safeToString(pkl['nisn']);
              final nama = _safeToString(pkl['nama']);
              final sekolah = _safeToString(pkl['sekolah']);
              final tMulai = _formatDateFrom(pkl['tanggalMulai']);
              final tBerakhir = _formatDateFrom(pkl['tanggalBerakhir']);
              final status = _safeToString(pkl['statusText']);
              return [
                index.toString(), // nomor urut
                nisn,
                nama,
                sekolah,
                tMulai,
            ...
            //   final tBerakhir = _formatDateFrom(pkl['tanggalBerakhir']);
            //   final status = _safeToString(pkl['statusText']);
            //   return [nisn, nama, sekolah, tMulai, tBerakhir, status];
            // }).toList(),
          ];
          ...
          return [
            ...
            pw.Table.fromTextArray(
              ...
              columnWidths: {
                ...
                // 0: const pw.FractionColumnWidth(0.12), // nomor
                // 1: const pw.FractionColumnWidth(0.20), // nama
                // 2: const pw.FractionColumnWidth(0.34), // judul (besar)
                // 3: const pw.FractionColumnWidth(0.12), // tanggal mulai
                // 4: const pw.FractionColumnWidth(0.12), // tanggal berakhir
                // 5: const pw.FractionColumnWidth(0.10), // status
              },
              cellAlignments: {
                0: pw.Alignment.center,
            ...
            pw.Column(
              ...
              children: [
                ...
                // pw.SizedBox(height: 4),
                pw.Text(
                  'Total Siswa PKL: ${rows.length}',
                  style: pw.TextStyle(fontSize: 10),
                ),
                pw.Text(
                  'Status: Diproses ($prosesCount) | Disetujui ($setujuCount) | Ditolak ($tolakCount)',
                  style: pw.TextStyle(fontSize: 10),
                ),
