...
class CetakLaporanPKSButton extends StatelessWidget {
  final List<dynamic> displayedRows;
  final String judulLaporan;
  final DateTime? startDate;
  final DateTime? endDate;

  const CetakLaporanPKSButton({
    Key? key,
    required this.displayedRows,
    this.judulLaporan = 'Laporan PKS',
    this.startDate,
    this.endDate,
  }) : super(key: key);
        ...
        build: (context) {
          ...
          final tableData = <List<String>>[
            ...
            ...rows.asMap().entries.map((entry) {
              ...
              final pks = entry.value;
              final nomor = _safeToString(pks['nomorMou']);
              final nomor2 = _safeToString(pks['nomorPks']);
              final judul = _safeToString(pks['judul']);
              final namaUnit = _safeToString(pks['namaUnit']);
              final tMulai = _formatDateFrom(pks['tanggalMulai']);
              final tBerakhir = _formatDateFrom(pks['tanggalBerakhir']);
              final status = _safeToString(pks['statusText']);
              return [
